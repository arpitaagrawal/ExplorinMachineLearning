		...
		for reg_coeff in reg_coeffs:
			for sgd_decay in sgd_decays:
				for sgd_mom in sgd_moms:
					...
					print('Score for architecture = '+ str(arch)+', lambda = '+str(reg_coeff)+' , decay = '+str(sgd_decay)+', ' +	
						'momentum = '+str(sgd_mom)+', actfn = '+str(actfn)+': '+str(score[1]))
	print('Best Config: architecture = '+str(best_config[0])+', lambda = '+str(best_config[1])+', decay = '+str(best_config[2])+', momentum = '+str(best_config[3])+', ' +
		'actfn = '+str(best_config[4])+', best_acc = '+str(best_config[5]))
	return best_config